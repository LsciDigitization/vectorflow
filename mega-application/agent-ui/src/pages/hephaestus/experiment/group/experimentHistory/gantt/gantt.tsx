...
import Block from '@/components/Gantt/block'
import Leftbar from '@/components/Gantt/leftbar'
import CurrentTime from '@/components/Gantt/markline/current-time'
import Markline from '@/components/Gantt/markline/index'
import type {GanttRow} from "@/pages/hephaestus/experiment/group/experimentHistory/gantt/ganttData";
interface ITimeline {
  time: string,
  color: string
}
...
interface IGanttProps {
  ...
  timelines: ITimeline[],
  scrollToTime?: string;
  renderLeftBar: (data: GanttRow) => JSX.Element;
  renderBlock: (data: GanttRow, getPositonOffset: (date: string) => number, getWidthAbout2Times: (time1: string, time2: string) => number, isInRenderingTimeRange: (time: string) => boolean, startTimeOfRenderArea: number, endTimeOfRenderArea: number) => JSX.Element;
...
class App extends React.Component<IGanttProps, IGanttState> {
  ...
  public componentDidMount = () => {
    const observeContainer = throttle((entries: any) => {
  ...
  // 同步fixleft和block的滚动
  public syncScrollY = (event: any, fake = false) => {
  ...
  public syncScrollX = (event: any, fake = false) => {
    const {
      blocksWrapperRef,
      headerTimelineRef,
      marklineAreaRef,
