...
export default function KeepAlivePage(props: any) {
  ...
  useEffect(() => {
    ...
    if (isExit < 0) {
      ...
      const isBeyondDistance =
        (showTabs as number) * (tabWidth as number) - (tabsWidth as number) + 100;
      const curClickIndex = tabList.findIndex(
        (item) => item.pathname === props.location.pathname,
      ) as number;
      // 能展示多少个
      ...
      if (curClickIndex > totalShowIndex) {
        ...
        if (isExit < transNum) {
          dispatch({
            type: 'CHANGESTATE',
            payload: {
              tarnslateX: tabWidth * curClickIndex,
              active: isExit,
            },
          });
        } else {
