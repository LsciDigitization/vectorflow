...
// const justExpireTime = (headers: any): void => {
...
const addHeaderAuthorization = (url?: string, options?: any) => {
  // console.log('-------------------------')
...
// 请求拦截器
request.interceptors.request.use(addHeaderAuthorization);
...
/**
 * 将current页数 复制给pageNum
 * @param url
 */
export function beforeHandler(url: string) {
  if (url?.indexOf('current') != -1) {
    const urlParams = getUrlParams(url);
    // eslint-disable-next-line no-param-reassign
    url += '&pageNum=' + urlParams.current
    return url;
  } else {
    return url;
  }
