...
import static com.mega.component.commons.URIComponents.AMPERSAND;
import static com.mega.component.commons.URIComponents.QUESTIONMARK;
...
import static com.mega.component.commons.net.URIUtil.decode;
import static com.mega.component.commons.net.URIUtil.encode;
...
import com.mega.component.commons.util.MapUtil;
...
public final class ParamUtil {
     ...
     * </blockquote>
     *
     * @param uriString   如果带有? 和参数,会先被截取,最后再拼接,<br>
     *                    如果不带?,则自动增加?
     * @param paramName   添加的参数名称
     * @param paramValue  添加的参数值
     * @param charsetType 字符编码,建议使用 {@link CharsetType} 定义好的常量<br>
     ...
     * @param uriString      如果带有? 和参数,会先被截取,最后再拼接,<br>
     *                       如果不带?,则自动增加?
     * @param singleValueMap singleValueMap param name 和value 的键值对
     * @param charsetType    字符编码,建议使用 {@link CharsetType} 定义好的常量<br>
     *                       <span style="color:green">如果是null或者 empty,那么参数部分原样返回,自行处理兼容性问题</span><br>
     *                       否则会先解码,再加码,因为ie浏览器和chrome浏览器 url中访问路径 ,带有中文情况下不一致
     * @return 如果 <code>uriString</code> 是null或者empty,返回 {@link StringUtils#EMPTY}<br>
     ...
     * Map{@code <String, String[]>} keyAndArrayMap = new LinkedHashMap{@code <>}();
     *
     * keyAndArrayMap.put("receiver", new String[] { "鑫哥", "test" });
     ...
     * @param uriString     如果带有? 和参数,会先被截取,最后再拼接,<br>
     *                      如果不带?,则自动增加?
     * @param arrayValueMap the name and array value map
     * @param charsetType   字符编码,建议使用 {@link CharsetType} 定义好的常量<br>
     *                      <span style="color:green">如果是null或者 empty,那么参数部分原样返回,自行处理兼容性问题</span><br>
     *                      否则会先解码,再加码,因为ie浏览器和chrome浏览器 url中访问路径 ,带有中文情况下不一致
     * @return 如果 <code>uriString</code> 是null或者empty,返回 {@link StringUtils#EMPTY}<br>
     ...
     * </blockquote>
     *
     * @param queryString {@code a=1&b=2}类型的数据,支持{@code a=1&a=1}的形式, 返回map的值是数组
     * @param charsetType 字符编码,建议使用 {@link CharsetType} 定义好的常量<br>
     *                    <span style="color:green">如果是null或者 empty,那么参数部分原样返回,自行处理兼容性问题</span><br>
     *                    否则会先解码,再加码,因为ie浏览器和chrome浏览器 url中访问路径 ,带有中文情况下不一致
     * @return 如果 <code>queryString</code> 是null或者empty,返回 {@link Collections#emptyMap()}<br>
     * @see org.apache.commons.lang3.ArrayUtils#add(Object[], Object)
     * @see com.mega.component.commons.lang.StringUtil#split(String, String)
     * @since 1.4.0
     */
     ...
     * </blockquote>
     *
     * @param singleValueMap 用于拼接签名的参数
     ...
     * </blockquote>
     *
     * @param singleValueMap the params map
     ...
     * @param singleValueMap the map
     * @param includeKeys    包含的key
     ...
     * 拼接url.
     *
     * @param beforePathWithoutQueryString the before path without query string
     * @param arrayValueMap                the array value map
     * @param charsetType                  字符编码,建议使用 {@link CharsetType} 定义好的常量<br>
     *                                     <span style="color:green">如果是null或者 empty,那么参数部分原样返回,自行处理兼容性问题</span><br>
     *                                     否则会先解码,再加码,因为ie浏览器和chrome浏览器 url中访问路径 ,带有中文情况下不一致
