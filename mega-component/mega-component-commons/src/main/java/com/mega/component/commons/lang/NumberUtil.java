...
import com.mega.component.commons.text.NumberFormatUtil;
 ...
 * </ol>
 * <p>
 * 在《Effective Java 》这本书中也提到这个原则,float和double只能用来做科学计算或者是工程计算,在商业计算中我们要用 {@link BigDecimal}.
 * </p>
 * </blockquote>
 *
...
public final class NumberUtil {
     ...
     * <blockquote>
     * <ol>
     * <li>是将两个数组转成 BigDecimal 类型,并调用 {@link BigDecimal#compareTo(BigDecimal)} 比较大小</li>
     * </ol>
     * </blockquote>
     *
     * <h3>示例:</h3>
     ...
     *
     * @param one   除数
     * @param two   被除数,自动转成{@link BigDecimal}做除法运算
     ...
     *                     如果为零或正数,则标度是小数点后的位数。<br>
     *                     如果为负数,则将该数的非标度值乘以 10 的负 scale 次幂 (通常情况用不到负数的情况)
     * @param roundingMode 舍入法 {@link RoundingMode}
     * @return 如果 <code>one</code> 是 null,抛出 {@link NullPointerException}<br>
     * 如果 <code>two</code> 是 null,抛出 {@link NullPointerException}<br>
     * 如果 <code>two</code> 是 0,抛出 {@link IllegalArgumentException}<br>
     ...
     * </blockquote>
     *
     * @param beSubtractedValue 被减数,如 100-10-5, 其中的100 就是被减数
     * @param subtractions      减数,如 100-10-5, 其中的10 和5 就是减数
     ...
     *
     * @param value         值
     * @param numberPattern 转成字符串格式 {@link NumberPattern}
