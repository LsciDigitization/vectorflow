...
@Slf4j
public class WorkStartPipette2Plate extends AbstractWorkStartPlate {

    public WorkStartPipette2Plate() {
    }

    public WorkStartPipette2Plate(WorkBusDaemonResource workBusDaemonResource) {
    ...
    public Optional<InstanceLabwareModel> startInstancePlate(long experimentHistoryId) {
        long workPipetteSize = getWorkPlateConsumer(GeneralPlateTypeEnum.PIPETTE2).size(experimentHistoryId);

        if (workPipetteSize > 0) {
            log.info("移液枪头2队列大小 {}", workPipetteSize);
            InstanceLabwareModel instancePlate = getWorkPlateConsumer(GeneralPlateTypeEnum.PIPETTE2).pop(experimentHistoryId);
    ...
    public Optional<InstanceLabwareModel> startInstancePlate(long experimentHistoryId, int plateNo) {
        ...
        if (workPipetteSize > 0) {
            log.info("移液枪头2队列大小 {}", workPipetteSize);
            InstanceLabwareModel instancePlate = getWorkPlateConsumer(GeneralPlateTypeEnum.PIPETTE2).pop(experimentHistoryId, plateNo);
            if (Objects.nonNull(instancePlate)) {
                this.accept(instancePlate);
                return Optional.of(instancePlate);
            }
        ...
        unfinishedInstanceSteps = unfinishedInstanceSteps.stream()
                .filter(a -> {
                    long c = instanceTasks.stream()
                            ...
                            .count();
                    log.info("启动移液枪头条件 instanceId {} step {} taskStatus {} {}", a.getInstanceId(), StepTypeEnum.STEP12.getCode(), InstanceTaskStatusEnum.Await.getValue(), c);
                    return c > 0;
                })
    ...
    public PlateType plateType() {
        return GeneralPlateTypeEnum.PIPETTE2;
    }
}
