...
@Slf4j
public class StartTaskExecutor extends AbstractStageFlowExecutor
{
    private final StartTaskExecutorResource startTaskExecutorResource;

    private final TaskLoggerService stageLoggerService;

    public StartTaskExecutor(int latchQty, StartTaskExecutorResource startTaskExecutorResource, TaskLoggerService stageLoggerService) {
        super(latchQty);
        this.startTaskExecutorResource = startTaskExecutorResource;
    ...
    public TaskExecutionResult doRun(ExecutableTask task) throws InterruptedException
    {
        ...
        Optional<InstanceEntity> instanceOptional = startTaskExecutorResource.getExperimentExecutorManager().getInstance(instanceId);
        instanceOptional.ifPresent(System.out::println);

        startTaskExecutorResource.getWorkEventPusher().sendInstanceStartedEvent(new InstanceStartedEvent(instanceId));
