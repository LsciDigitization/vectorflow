 ...
 */
@Slf4j
public class ConditionTaskExecutor extends AbstractStageFlowExecutor {

    private final ConditionTaskExecutorResource conditionTaskExecutorResource;

    private final TaskLoggerService stageLoggerService;

    public ConditionTaskExecutor(int latchQty, ConditionTaskExecutorResource conditionTaskExecutorResource, TaskLoggerService stageLoggerService) {
        super(latchQty);
        this.conditionTaskExecutorResource = conditionTaskExecutorResource;
    ...
    public TaskExecutionResult doRun(ExecutableTask task) throws InterruptedException {
        ...
        Optional<InstanceEntity> instanceOptional = conditionTaskExecutorResource.getExperimentExecutorManager().getInstance(instanceId);
        ...
        try {
            instanceOptional.ifPresentOrElse(hephaestusInstance -> {
                ...
                // 查找当前stage阶段数据
                Optional<HephaestusStage> stagesOptional = conditionTaskExecutorResource.getExperimentExecutorManager().getStage(hephaestusInstance, task.getTaskId().getId());
                stagesOptional.ifPresentOrElse(v -> {
                    ...
                    List<HephaestusStageTask> stageTasks = conditionTaskExecutorResource.getExperimentStageTaskManager().getStageTasks(hephaestusInstance.getExperimentId(), task.getTaskId().getId());
...
//                    // 运行阶段子任务
//                    // 设置线程池
//                    conditionTaskExecutorResource.getDynamicStageTask().setDeviceLockExecutor(conditionTaskExecutorResource.getDeviceLockExecutor());
//                    conditionTaskExecutorResource.getDynamicStageTask().run(runParameter);
...
//                    runParameter.setTasks(stageTasks);
                    ...
                    // 运行阶段子任务
                    // 设置线程池
                    conditionTaskExecutorResource.getDynamicStage().setDeviceLockExecutor(conditionTaskExecutorResource.getDeviceLockExecutor());
                    conditionTaskExecutorResource.getDynamicStage().run(runParameter);
            ...
            conditionTaskExecutorResource.getExperimentInstanceManager().failStage(instanceId, currentStage.get().getId());
        ...
        // stage执行完成，保存数据
        conditionTaskExecutorResource.getExperimentInstanceManager().endStage(instanceId, currentStage.get().getId());
